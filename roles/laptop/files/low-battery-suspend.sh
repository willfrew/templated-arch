...
# FIXME less hacky way of grabbing this
export DBUS_SESSION_BUS_ADDRESS="unix:path=/run/user/$(id -u $WF_USER)/bus"
...
if [[ $(cat /sys/class/power_supply/$WF_AC_CHARGING_FILE) -ne "1" ]]; then
  ...
  if [[ $(cat /sys/class/power_supply/$WF_BATTERY_NAME/capacity) -eq $WF_LOW_BATTERY_NOTIFICATION_PERCENTAGE ]]; then
    su $WF_USER --command "notify-send --urgency 'critical' 'Low Battery $WF_LOW_BATTERY_NOTIFICATION_PERCENTAGE%'"
  fi
else
  ...
  if [[ $(cat /sys/class/power_supply/$WF_BATTERY_NAME/capacity) -gt 99 ]]; then
    su $WF_USER --command "notify-send --urgency 'critical' --hint 'string:frcolor:#859900' 'Battery Fully Charged' 'Unplug from power'"
  fi
fi
